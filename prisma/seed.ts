...
async function main() {
  ...
  // 9) Demo dataset — opt-in via SEED_DEMO=1; populates business-dashboard. Idempotent via sentinel DEMO-0001.
  if (process.env.SEED_DEMO) {
    const sentinel = await db.order.findUnique({ where: { orderNo: "DEMO-0001" } }).catch(() => null);
    if (sentinel) {
      console.log("  demo: already seeded (DEMO-0001 exists), skipping");
    } else {
      const daysAgo = (n: number) => new Date(NOW.getTime() - n * 86400000);
      const demoUsers = [];
      for (let i = 1; i <= 8; i++) {
        demoUsers.push(await db.user.upsert({
      ...
      console.log(`  demo: ${demoOrders.length} orders + 40 quiz + 60 events + funnel (SEED_DEMO)`);
    }
  }
}
