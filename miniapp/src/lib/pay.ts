...
export async function payWechatOrder(orderId: string, accessToken: string): Promise<void> {
 ...
 */
export async function payXhsOrder(orderId: string, accessToken: string): Promise<void> {
  ...
  if (xhs && typeof xhs.requestPayment === "function") {
    await new Promise<void>((resolve, reject) => {
      xhs.requestPayment({
        ...p,
        success: () => resolve(),
        fail: (e: any) => reject(new Error(e?.errMsg || "支付失败")),
      });
    });
    return;
  }
...
export async function payOrder(platform: string, orderId: string, accessToken: string): Promise<void> {
  if (platform === "xhs") return payXhsOrder(orderId, accessToken);
  return payWechatOrder(orderId, accessToken);
}
