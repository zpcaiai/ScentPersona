...
### 3.6 对象存储 · Hero 图上传（选填）

`S3_ENDPOINT` `S3_BUCKET` `S3_ACCESS_KEY_ID` `S3_SECRET_ACCESS_KEY`（S3 兼容：AWS S3 / Cloudflare R2 / MinIO / B2），可选 `S3_REGION`（默认 auto）、`S3_PUBLIC_BASE_URL`（CDN 公开读取基址）、`S3_FORCE_PATH_STYLE`（默认 true）。配好后在后台内容页点「上传图片」即可设为 hero；未配置时按钮提示未配置，仍可手填 URL。上传走服务端 SigV4 预签名（`POST /api/admin/uploads/presign`，需 `product:edit`），密钥不落前端。前端校验格式（JPG/PNG/WebP）、大小（`MAX_UPLOAD_BYTES` 默认 5MB）与尺寸（200–6000px），并自动生成缩略图一并上传（`.thumb` 同名约定，列表页自动用缩略图）；服务端用预签名 POST 的 `content-length-range` 在存储端强制大小上限，并对预签名接口按操作员限流（20 次/分）。内容页新增「商品聚合块生成器」（可视化选香型/选商品）与「内容块拖拽排序」（拖拽/上下移/删除整段块），基本免手写 JSON。

`ENABLE_DEMO_TOOLS=1` 时经营看板出现「重置演示数据」按钮（`POST /api/admin/demo/reset`，需 `admin:manage`）。

限流（可选）：配置 `UPSTASH_REDIS_REST_URL` + `UPSTASH_REDIS_REST_TOKEN` 后，预签名等接口用 Upstash Redis 做**跨实例**限流；**未配置或 Redis 不可达时自动降级为进程内内存限流，系统照常工作**（无 Redis 也能用）。
...
> 拉取本次改动后（新增 `heroThumbUrl` 列）务必先 `npx prisma generate` 再 `npx prisma migrate deploy`，缩略图落库才会生效；生成前该列不写入、列表页自动回退到 `.thumb` 约定推导，不影响使用。

迁移顺序（`prisma/migrations/`）：
`init` → `add_orders` → `secure_orders` → `order_fulfillment` → `feedback_order_link` → `data_deletion_requests` → `analytics_events` → `catalog_products` → `proxy_orders` → `commercial_infra` → `trust_compliance_risk` → `support_notify_inventory_fulfillment` → `cms_coupons_conversion_wardrobe` → `admin_auth` → `content_hero_thumb`。
...
```bash
npm run reset:demo                   # 预览将删除的数量（tsx，复用共享逻辑）
npm run reset:demo -- --yes          # 执行删除并重新播种
```

也可在后台经营看板点「重置演示数据」按钮（设 `ENABLE_DEMO_TOOLS=1` 后可见，需 `admin:manage` 角色）。
...
前台新增 `/c` 专题聚合页（列出所有已发布专题），首页也有"按香型探索 + 精选专题"推荐位，数据均来自数据库种子。每个专题页带 hero 图（种子用 picsum 占位；后台内容页可「上传图片」或手填 URL）和「自营/推荐商品聚合位」。聚合位由内容块 JSON 驱动，三种指定方式：按香型 `{"type":"products","family":"木质东方"}`、按商品名 `{"type":"products","productNames":["白茶清晨淡香水"]}`、或按 offer id `{"type":"products","offerIds":["..."]}`；自营（flagship/official）优先展示。
