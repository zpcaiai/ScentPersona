...
`S3_ENDPOINT` `S3_BUCKET` `S3_ACCESS_KEY_ID` `S3_SECRET_ACCESS_KEY`（S3 兼容：AWS S3 / Cloudflare R2 / MinIO / B2），可选 `S3_REGION`（默认 auto）、`S3_PUBLIC_BASE_URL`（CDN 公开读取基址）、`S3_FORCE_PATH_STYLE`（默认 true）。配好后在后台内容页点「上传图片」即可设为 hero；未配置时按钮提示未配置，仍可手填 URL。上传走服务端 SigV4 预签名（`POST /api/admin/uploads/presign`，需 `product:edit`），密钥不落前端。
